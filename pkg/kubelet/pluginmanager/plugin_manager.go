...
func NewPluginManager(
	...
	// if Run() is never called, as documented in the interface.
	close(pm.stopped)
	return pm
}
...
type pluginManager struct {
	...
	// It's created closed and reopened when Run() is called.
	stopped chan struct{}
...
func (pm *pluginManager) Run(ctx context.Context, sourcesReady config.SourcesReady, stopCh <-chan struct{}) {
	pm.runOnce.Do(func() {
		...
		// This allows Stopped() to work correctly whether Run() is called or not.
		pm.stoppedMu.Lock()
		pm.stopped = make(chan struct{})
		stoppedCh := pm.stopped
		pm.stoppedMu.Unlock()
